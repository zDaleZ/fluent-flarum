const Menu = (parent, [...links], options = {}) => {
    const isInViewport = () => {
        ...
        if (firstElement) rectFirstElement = parent.firstElementChild.getBoundingClientRect();
        return (
            rectMenu.left > 0 &&
            rectLastElement.right <= (window.innerWidth || document.documentElement.clientWidth) &&
    ...
    const isFreeSpace = () => {
        ...
        const freeSpaceBetween = siblingLeft ? wrapperRect.left - rectSiblingLeft.right : wrapperRect.left;
        const freeSpaceLeft = firstElement ? rectFirstElement.left : wrapperRect.left;
        const linkWidth = dropdownList.lastElementChild.clientWidth;

        return freeSpaceRight >= linkWidth || freeSpaceBetween >= linkWidth || freeSpaceLeft >= linkWidth;
    };
    ...
    const createDropdown = (wrapper) => {
        ...
        title.textContent = typeof options.dropdownText === 'string' ? options.dropdownText : 'More ↓';
